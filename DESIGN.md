...
文档站的视觉系统。配套实现见 `src/styles/` 与 `astro.config.ts`（`markdown.theme`）。设计语言为 **zinc 单色系**，目标是「毫不费力的长文阅读」。任何新增页面 / 组件都应回到本文件对齐，再按 UX 需要外扩。
...
强调色刻意映射为墨黑 / 墨白阶梯（浅色 `#18181b → #52525b`，深色 `#fafafa → #a1a1aa`），不引入蓝色等默认链接色。
...
- **阅读栏宽度 720px** —— 控制行长在舒适区间，是该设计的核心约束之一。
- 整体最大宽 1440px。
