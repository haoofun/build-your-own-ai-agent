...
> 本文只描述最终成品必须具备的能力和最小实现边界，是规划各 Part 的上游基线。`OUTLINE.md` 定义 Part 目标与暂定教学展开，`SPIKE-NOTES.md` 记录实测约束。章数和章名与本文冲突时调整章节；新的实测证据推翻本文时，先更新本文再继续规划。
...
三档图不是三套架构，而是同一套代码在不同 Part 结束时的真实截面。保留它们是为了让读者看到系统怎样长出来，也让求职面试官能快速判断项目深度；每张图只出现读者已经实现的概念。
...
此时再展示环境、权限、上下文管理、取消和 Session 保存；它们已经在第二、第三部分逐步出现。
...
| 单会话 agent 图 | 第三部分结束 | 收束环境、权限、上下文、取消和持久化 |
| 最终能力图 | 第五部分结束、README | 展示 Todo、Skills、子 agent 和 MCP 如何复用同一核心 |
...
当前只实现只读子 agent。写操作子 agent 和 git worktree 隔离放在正文完成后的优先番外，不进入正文练习的完成要求。
...
- 番外使用独立文章和 git tag，不反向改变已经发布并冻结的正文 checkpoint。
...
本文描述的是正文主线结束时的形状，不是第一章的脚手架。课程用两条互相咬合的线推进：

```text
真实任务需求 → 新能力 → 新的工程压力 → runtime 责任 → 可验证的阶段出口
```

Part 由真实任务需求划分；章由实现该 Part 时实际出现的工程问题和可运行 checkpoint 反推。每个抽象必须由读者已经遇到的问题引出，没有当前问题支撑的抽象不提前加入。

| 阶段 | 真实需求 | 新能力造成的工程压力 | 本阶段新增的 runtime 责任 | 出口证据 |
|---|---|---|---|---|
| 序章 · 看见模型边界 | 先从终端调用模型 | 模型和客户端都没有跨轮状态 | 裸 fetch、单句 message、无状态对话壳 | 连问“我叫什么”稳定暴露失忆 |
| 第一部分 · 核心闭环 | 不只回答，而是修改环境并完成任务 | tool 协议、消息配对、循环终止、本地副作用、裸类型维护 | `runAgent`、`callModel`、Tool、`ExecutionEnv`、Read / Write / Edit / Bash / Grep，最后 fetch → SDK | agent 把失败测试修到通过；fetch / SDK transcript 等价 |
| 第二部分 · 可控运行 | 在真实项目里安全、可观察地执行 | 环境未知、危险操作、流式 partial、工具失败、网络波动和孤儿进程 | system prompt、项目指令、`onEvent`、权限、`ToolResult.isError`、重试、`RunResult`、Abort | 遵守项目约定；拒绝和失败可恢复；Ctrl+C 无孤儿进程 |
| 第三部分 · 长任务与 Session | 处理长任务并跨 prompt 延续 | messages 膨胀、成本不可见、长输出、信息丢失和进程重启 | Usage、截断、caching、compaction、线性 `SessionState`、JSONL | 压缩后保留原始任务；cache 可观测；关闭重开恢复 |
| 第四部分 · 计划与分工 | 完成更多步骤、调查更大代码库 | 多步任务漏项，搜索证据污染主上下文 | Todo、Task Tool、只读 Tool Map、usage 上卷、证据约束和一层深度限制 | Todo 对比不漏步骤；子 agent 返回可复查证据 |
| 第五部分 · 开放扩展 | 不改内核也能增加专业行为和外部工具 | prompt 常驻浪费 token，远程工具有命名、权限、错误和进程生命周期 | 按需 Skills、MCP 注册与调用；统一进入现有 system prompt / Tool Map | 未加载 Skill 不占 token；真实 MCP server 可调用并干净关闭 |
| 第六部分 · 验证与交付 | 让别人相信、安装和复现 | 体验粗糙、改进靠感觉、能力边界容易夸大 | Renderer、确定性 eval、证据矩阵、打包、发布 CI | GIF、通过率与成本报告、干净环境三分钟 demo |

三档架构图是这条演进线的能力快照，不负责决定 Part 数量：核心循环图放第一部分末，单会话图放第三部分末，最终能力图放第五部分末；第六部分再用“BYOAA → 成熟 coding agent”能力地图解释生产差距。

章级落点表不再长期保存在本文。开始一个 Part 前，先粗跑通该阶段出口并补必要 mini-spike，再从真实代码增量重建该 Part 的章节表；冻结后的行约束当前实现，发布后的 slug 与 tag 成为对读者稳定的接口。

流式与 `onEvent` 的形状仍是唯一没有经过 spike 周实测的核心项。进入第二部分前，需要先跑一次串通「流式 → 事件 → 最小 renderer」的 mini-spike，否则它可能连带返工后续 Session、终端显示和 eval。
